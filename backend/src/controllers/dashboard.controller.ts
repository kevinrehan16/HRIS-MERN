...
const labelFor = (date: Date) => date.toLocaleDateString('en-US', { weekday: 'short', timeZone: 'UTC' });

export const getAdminDashboard = catchAsync(async (_req, res: Response) => {
...
export const getEmployeeDashboard = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const today = getPHStartOfDay();
  const monthStart = new Date(today);
  monthStart.setUTCDate(1);
