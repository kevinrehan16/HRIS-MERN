...
export const calculateLeaveDays = ({ startDate, endDate, isHalfDay, restDays, holidayDates }) => {
    ...
    while (cursor <= endDate) {
        ...
        if (!isRestDay && !holidaySet.has(toDateKey(cursor)))
            days += 1;
        cursor.setUTCDate(cursor.getUTCDate() + 1);
    }
    if (days === 0)
        throw new Error('The selected dates do not include a scheduled workday.');
    return isHalfDay ? 0.5 : days;
};
//# sourceMappingURL=leave.util.js.map