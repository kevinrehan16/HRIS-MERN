...
const optionalId = (value) => value === undefined || value === null || value === '' ? undefined : Number(value);
...
export const register = catchAsync(async (req, res) => {
    ...
    if (existingEmployee)
        throw new AppError('An employee account or statutory ID is already registered.', 409);
    if (uniqueValues.length !== new Set(uniqueValues).size)
        throw new AppError('Statutory IDs must be unique for each employee.', 400);
    const birthDate = body.birthDate ? new Date(body.birthDate) : undefined;
    if (birthDate && Number.isNaN(birthDate.getTime()))
        throw new AppError('Birth date must be valid.', 400);
...
export const login = catchAsync(async (req, res) => {
    ...
    const token = jwt.sign({ id: employee.id, email: employee.email, role: employee.role }, process.env.JWT_SECRET || 'supersecretkey', { expiresIn: '1d' });
