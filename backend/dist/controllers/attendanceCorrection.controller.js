...
export const approveCorrection = catchAsync(async (req, res, next) => {
    ...
    const mergeTimeWithOriginalDate = (originalDate, newTimeSource) => {
        if (!newTimeSource)
            return originalDate;
    ...
    const parseShiftMinutes = (timeStr) => {
    ...
    sendNotification(correction.employeeId, 'Correction Approved', `Your request for Attendance Correction has been approved.`).catch(console.error);
...
//# sourceMappingURL=attendanceCorrection.controller.js.map