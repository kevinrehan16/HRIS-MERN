...
export const timeOut = catchAsync(async (req, res) => {
    ...
    if (!attendance)
        return res.status(400).json({ success: false, message: "No active time-in found." });
    ...
    if (schedule) {
        ...
        if (eHour < sHour)
            shiftEnd.setDate(shiftEnd.getDate() + 1); // Night Shift
...
export const adjustAttendance = catchAsync(async (req, res) => {
    ...
    if (tOut) {
        ...
        if (eHour < sHour)
            targetEnd.setDate(targetEnd.getDate() + 1); // Night shift fix
        if (tOut < targetEnd) {
...
export const getAdminAttendance = catchAsync(async (req, res) => {
    const { date } = req.query; // Pwedeng mag-pass ng ?date=2026-04-15
    const logs = await getAdminAttendanceLogs(date);
...
export const getPendingOvertime = catchAsync(async (req, res) => {
