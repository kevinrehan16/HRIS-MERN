...
const toAppError = (error) => new AppError(error instanceof Error ? error.message : 'Unable to calculate leave duration.', 400);
const getLeaveCalendar = async (employeeId, startDate, endDate) => {
    ...
    if (!employee.schedule)
        throw new AppError('A work schedule must be assigned before filing leave.', 400);
...
export const getAllLeaveRequests = catchAsync(async (req, res) => {
    ...
    if (status !== 'ALL' && !leaveStatuses.includes(status)) {
...
export const applyLeave = catchAsync(async (req, res) => {
    ...
    if (overlapping)
        throw new AppError('This request overlaps an existing pending or approved leave request.', 409);
...
export const updateLeaveStatus = catchAsync(async (req, res) => {
    ...
    if (leave.status !== 'PENDING')
        throw new AppError('Only pending leave requests can be decided.', 409);
    ...
    await sendNotification(leave.employeeId, status === 'APPROVED' ? 'Leave request approved' : 'Leave request rejected', status === 'APPROVED'
...
export const cancelMyLeave = catchAsync(async (req, res) => {
    const leaveId = Number(req.params.id);
    const leave = await prisma.leaveRequest.findFirst({ where: { id: leaveId, employeeId: req.user.id } });
    if (!leave)
        throw new AppError('Leave request not found or not yours.', 404);
